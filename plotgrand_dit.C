...
void plotgrand_dit(){
    ...
    int a_slug = 26;
    int b_slug = 54;
    ch->Add(Form("./rootfiles/dithering/slugs/slugAvg/grand_slug%s.root",key.Data()));
    ofstream outfile(Form("./TextFile/grand_dithering_slug%d_%d.csv",a_slug,b_slug));
...
//    TString cuts[] = {"arm==0&&wein==1&&ihwp==1&&slug<=40&&slug>=30","arm==0&&wein==2&&ihwp==2&&slug<=40&&slug>=30","arm==0&&wein==1&&ihwp==2&&slug<=40&&slug>=30","arm==0&&wein==2&&ihwp==1&&slug<=40&&slug>=30"};
   ...
   Int_t  npt = ch->Draw(Form("%d*%s.mean:%s.err:%s.rms:slug:Entry$",sign[icuts],device.Data(),device.Data(),device.Data()),cuts[icuts].Data(),"goff");
    ...
    mg->Add(gr[2]);
    mg->Add(gr[3]);
    mg->SetTitle(Form("%s_mean/ppm;%s;%s_mean/ppm",device.Data(),"slug",device.Data()));
    mg->Draw("AP");
    mg->Fit("pol0");
...
//    outfile<<device.Data()<<"\t"<<Form("%4.3f \t %4.3f \t %4.1f/%d",mean[0],error[0],ChiSq[0],NDF[0])<<"\t"<<Form("%4.3f \t %4.3f \t %4.1f/%d",mean[1],error[1],ChiSq[1],NDF[1])<<"\t"<<Form("%4.3f \t %4.3f \t %4.1f/%d",mean[2],error[2],ChiSq[2],NDF[2])<<"\t"<<Form("%4.3f \t %4.3f \t %4.1f/%d",mean[3],error[3],ChiSq[3],NDF[3])<<"\t"<<Form("%4.3f \t %4.3f \t %4.1f/%d",Gfit->GetParameter(0),Gfit->GetParError(0),Gfit->GetChisquare(),Gfit->GetNDF())<<endl;
    ...
    mgRMS->Add(grRMS[2]);
    mgRMS->Add(grRMS[3]);
    mgRMS->SetTitle(Form("%s_rms/ppm;%s;%s_rms/ppm",device.Data(),"slug",device.Data()));
    mgRMS->Draw("AP");
    mgRMS->Fit("pol0");
...
//      textA[i]->Draw();
    ...
    npull = ch->Draw(Form("%s.mean:%s.err:slug:Entry$",device.Data(),device.Data()),Form("slug>=%d&&slug<=%d",a_slug,b_slug),"goff");
    ...
    hPull->Fill(i,val[i]);
    hPull1D->Fill(val[i]);
//    cout<<point[i]<<"\t"<<Gfit->GetParameter(0)<<"\t"<<sigma[i]<<"\t"<<val[i]<<endl;
    }
    ...
    hPull->SetStats(0);
    gPad->SetGrid();
    hPull->SetTitle(Form("%s_pull/ppm;slug;%s_pull/ppm",device.Data(),device.Data()));
