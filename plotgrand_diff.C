void plotgrand_diff(){
    ...
    TString key = "26_54";
    ch->Add(Form("./rootfiles/postpan/slugs/slugAvgBPM/grand_slug%s.root",key.Data()));
    ofstream outfile("./TextFile/grand_postpan_diff_bpm_slug26_54.csv");
...
//    TString cuts[] = {"arm==0&&wein==1&&ihwp==1&&slug<=40&&slug>=30","arm==0&&wein==2&&ihwp==2&&slug<=40&&slug>=30","arm==0&&wein==1&&ihwp==2&&slug<=40&&slug>=30","arm==0&&wein==2&&ihwp==1&&slug<=40&&slug>=30"};
   ...
   Int_t  npt = ch->Draw(Form("%d*%s.mean:%s.err:%s.rms:slug:Entry$",sign[icuts],device_name[iDL].Data(),device_name[iDL].Data(),device_name[iDL].Data()),cuts[icuts].Data(),"goff");
    ...
    mg->Add(gr[2]);
    mg->Add(gr[3]);
    mg->SetTitle(Form("diff_%s_mean/um;%s;diff_%s_mean/um",device_name[iDL].Data(),"slug",device_name[iDL].Data()));
    mg->Draw("AP");
    mg->Fit("pol0");
...
//      textA[i]->Draw();
    ...
    mgRMS->Add(grRMS[2]);
    mgRMS->Add(grRMS[3]);
    mgRMS->SetTitle(Form("diff_%s_rms/um;%s;diff_%s_rms/um",device_name[iDL].Data(),"slug",device_name[iDL].Data()));
    mgRMS->Draw("AP");
    mgRMS->Fit("pol0");
...
//      if(icuts==1||icuts==3){
    ...
    average = Form("Grand: %4.5f#pm%4.5f #chi^{2}: %4.1f/%d",GfitRMS->GetParameter(0),GfitRMS->GetParError(0),GfitRMS->GetChisquare(),GfitRMS->GetNDF());
...
//      textA[i]->Draw();
    ...
    outfile<<device_name[iDL].Data()<<"\t"<<Form("%4.5f \t %4.5f \t %4.1f/%d \t %4.5f",Gfit->GetParameter(0),Gfit->GetParError(0),Gfit->GetChisquare(),Gfit->GetNDF(),GfitRMS->GetParameter(0))<<endl;
    c1->SaveAs(Form("plots/postpan/diffBPM/device_%d_rms.pdf",iDL));
    }
    gSystem->Exec(Form("pdfunite ./plots/postpan/diffBPM/device_0_*.pdf ./plots/postpan/diffBPM//device_1_*.pdf ./plots/postpan/diffBPM/device_2_*.pdf ./plots/postpan/diffBPM/device_3_*.pdf ./plots/postpan/diffBPM/device_4_*.pdf ./plots/postpan/diffBPM/grand_AT_%s.pdf","26-54"));
