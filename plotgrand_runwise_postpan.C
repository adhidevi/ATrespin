...
void plotgrand_runwise_postpan(){
	gStyle->SetOptFit(0);
	gStyle->SetTitleYOffset(1.2);
	TChain* ch = new TChain("mini");
	ch->Add(Form("./rootfiles/postpan/slugs/grand_slug26_94.root"));
	int a_slug = 30;
	int b_slug = 40;
	ofstream outfile(Form("./TextFile/grand_postpan_runwise_slug%d_%d.csv",a_slug,b_slug));
...
//	TCut cuts[] = {"arm==0&&wein==1&&ihwp==1&&slug<55","arm==0&&wein==2&&ihwp==2&&slug<55","arm==0&&wein==1&&ihwp==2&&slug<55","arm==0&&wein==2&&ihwp==1&&slug<55"};
	...
	int npt = ch->Draw(Form("%d*1e6*%s.mean:1e6*%s.err:1e6*%s.rms:run:Entry$",sign[icuts],device_name[iDL].Data(),device_name[iDL].Data(),device_name[iDL].Data()),user_cut+cuts[icuts],"goff");
	...
	mg->Add(gr[2]);
	mg->Add(gr[3]);
	mg->SetTitle(Form("%s_mean/ppm;run(minirun);%s_mean/ppm",device_name[iDL].Data(),device_name[iDL].Data()));
	mg->Draw("AP");
	mg->Fit("pol0");
...
//	if(icuts<ncuts){
	...
	mgRMS->Add(grRMS[2]);
	mgRMS->Add(grRMS[3]);
	mgRMS->SetTitle(Form("%s_rms/ppm;run(minirun);%s_rms/ppm",device_name[iDL].Data(),device_name[iDL].Data()));
	mgRMS->Draw("AP");
	mgRMS->Fit("pol0");
...
//	if(icuts<ncuts){
	...
	npull = ch->Draw(Form("1e6*%s.mean:1e6*%s.err:slug:Entry$",device_name[iDL].Data(),device_name[iDL].Data()),Form("slug>=%d&&slug<=%d",a_slug,b_slug)+user_cut,"goff");
	...
	hPull->SetStats(0);
	hPull->SetTitle(Form("%s_pull/ppm;run(minirun);%s_pull/ppm",device_name[iDL].Data(),device_name[iDL].Data()));
